...
const allowedEnvironmentNames = new Set(['PORT', 'BACKEND_API_URL', 'VITE_APP_BASE_PATH', 'VITE_API_BASE_URL'])
const environmentNames = environmentExample
  .split(/\r?\n/)
  .filter((line) => /^[A-Z][A-Z0-9_]*=/.test(line))
  .map((line) => line.slice(0, line.indexOf('=')))
const unexpectedEnvironmentNames = environmentNames.filter((name) => !allowedEnvironmentNames.has(name))
if (unexpectedEnvironmentNames.length) {
  throw new Error(`Unexpected Frontend variables: ${unexpectedEnvironmentNames.join(', ')}`)
}
