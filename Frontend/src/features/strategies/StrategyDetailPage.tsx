...
export function StrategyDetailPage() {
  ...
  return <div className="page-stack">
    ...
    <div className="strategy-control-bar"><div><StatusBadge status={item.enabled ? 'ENABLED' : 'DISABLED'} /><StatusBadge status={item.execution_mode} /><StatusBadge status={item.state} />{item.conid ? <StatusBadge status="CONTRACT QUALIFIED" /> : <StatusBadge status="CONTRACT PENDING" />}</div><div><button className="button-secondary" disabled={!canEnable(item) || action.isPending || deleteAction.isPending} onClick={() => action.mutate({name: 'enable'})}><Power />Enable</button><button className="button-secondary" disabled={!canPause(item) || action.isPending || deleteAction.isPending} onClick={() => action.mutate({name: 'pause'})}><CirclePause />Pause</button><button className="button-danger-subtle" aria-label={`Delete ${item.name}`} disabled={action.isPending || deleteAction.isPending} onClick={() => setDeleteOpen(true)}><Trash2 />Delete</button><button className="button-danger-subtle" disabled={!canFlatten(item) || action.isPending || deleteAction.isPending} onClick={() => setFlattenOpen(true)}><SlidersHorizontal />Flatten target</button></div></div>
