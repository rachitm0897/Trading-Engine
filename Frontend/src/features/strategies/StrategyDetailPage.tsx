...
function OverviewTab({strategy, timeline, timelineLoading}: {strategy: StrategyInstance; timeline: StrategyTimelineItem[]; timelineLoading: boolean}) {
  ...
  return <div className="page-stack">
    ...
    <div className="detail-grid">
      <Panel title="Readiness" description="Contract, warm-up, and execution state"><dl className="detail-list"><div><dt>Contract</dt><dd>{strategy.conid ? <><StatusBadge status="QUALIFIED" /><code>{strategy.conid}</code></> : <StatusBadge status="PENDING" />}</dd></div><div><dt>Warm-up</dt><dd><div className="wide-progress"><span>{strategy.warmup_progress} / {strategy.warmup_required} bars</span><div><i style={{width: `${warmup * 100}%`}} /></div></div></dd></div><div><dt>State</dt><dd><StatusBadge status={strategy.state} /></dd></div><div><dt>Mode</dt><dd><StatusBadge status={strategy.execution_mode} /></dd></div><div><dt>Last final bar</dt><dd>{formatDateTime(strategy.last_final_bar)}</dd></div></dl></Panel>
