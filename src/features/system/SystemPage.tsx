...
export function SystemPage() {
  ...
  const researchMVP = useQuery(queries.researchMVPStatus())
  ...
  const refresh = async () => {await Promise.all(['system', 'gateway', 'streaming', 'reconciliation', 'risk', 'audit', 'finnhub', 'research-mvp-status'].map((key) => queryClient.invalidateQueries({queryKey: [key]})))}
  ...
  const newest = Math.max(system.dataUpdatedAt, gateway.dataUpdatedAt, streaming.dataUpdatedAt, reconciliation.dataUpdatedAt, risk.dataUpdatedAt, audit.dataUpdatedAt, finnhub.dataUpdatedAt, researchMVP.dataUpdatedAt)
  ...
  return <div className="page-stack">
    ...
    <section className="metric-grid compact"><TerminalMetric label="Application mode" value={<StatusBadge status={system.data?.mode || 'UNKNOWN'} />} /><TerminalMetric label="IBKR connection" value={<StatusBadge status={gateway.data?.connected ? 'CONNECTED' : 'DISCONNECTED'} />} /><TerminalMetric label="Reconciliation" value={<StatusBadge status={gateway.data?.reconciled ? 'RECONCILED' : 'BLOCKED'} />} /><TerminalMetric label="Research MVP" value={<StatusBadge status={researchMVP.data?.research_enabled && researchMVP.data?.mvp_enabled ? 'ENABLED' : 'DISABLED'} />} helper={`${researchMVP.data?.ready_stock_count || 0}/5 stocks ready`} /><TerminalMetric label="Material breaks" value={formatNumber(system.data?.material_breaks)} /><TerminalMetric label="Global kill switch" value={<StatusBadge status={globalSwitch ? 'ENGAGED' : 'CLEAR'} />} /></section>
    <div className="system-sections">
      ...
      <TerminalPanel id="research-mvp-status" title="Recommendation MVP" description="Read-only operational status. Bootstrap and promotion mutations remain command-line only." badge={<StatusBadge status={researchMVP.data?.completed_experiment_groups === 25 ? 'READY' : 'BLOCKED'} />}>
        {researchMVP.isLoading ? <Skeleton lines={3} /> : researchMVP.isError ? <ErrorState title="Research MVP status is unavailable" error={researchMVP.error} onRetry={() => void researchMVP.refetch()} compact /> : <div className="page-stack"><section className="metric-grid compact"><TerminalMetric label="Research" value={<StatusBadge status={researchMVP.data?.research_enabled ? 'ENABLED' : 'DISABLED'} />} /><TerminalMetric label="Finnhub" value={<StatusBadge status={researchMVP.data?.finnhub?.configured ? 'CONFIGURED' : 'MISSING'} />} /><TerminalMetric label="IBKR" value={<StatusBadge status={researchMVP.data?.ibkr?.connected ? 'CONNECTED' : 'DISCONNECTED'} />} /><TerminalMetric label="Pilot stocks" value={`${researchMVP.data?.ready_stock_count || 0} / 5`} /><TerminalMetric label="Experiment groups" value={`${researchMVP.data?.completed_experiment_groups || 0} / 25`} /><TerminalMetric label="Eligible candidates" value={formatNumber(researchMVP.data?.eligible_candidate_count)} /></section><dl className="detail-list columns"><div><dt>Last data refresh</dt><dd>{formatDateTime(researchMVP.data?.last_data_refresh)}</dd></div><div><dt>Last experiment run</dt><dd>{formatDateTime(researchMVP.data?.last_experiment_run)}</dd></div></dl></div>}
      </TerminalPanel>
