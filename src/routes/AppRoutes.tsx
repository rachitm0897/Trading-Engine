...
const ResearchUniversePage = lazy(() => import('../features/research/ResearchUniversePage').then((module) => ({default: module.ResearchUniversePage})))

export function AppRoutes() {
  return <Suspense fallback={<Skeleton lines={6} height={420} />}><Routes><Route element={<AppShell />}><Route index element={<Navigate to="/dashboard" replace />} /><Route path="dashboard" element={<DashboardPage />} /><Route path="strategies" element={<StrategiesPage />} /><Route path="strategies/new" element={<CreateStrategyPage />} /><Route path="strategies/:strategyId" element={<StrategyDetailPage />} /><Route path="research" element={<ResearchUniversePage />} /><Route path="portfolio-builder" element={<PortfolioBuilderPage />} /><Route path="portfolio" element={<PortfolioPage />} /><Route path="activity" element={<OrdersActivityPage />} /><Route path="system" element={<SystemPage />} /><Route path="*" element={<NotFoundPage />} /></Route></Routes></Suspense>
}
