...
def check_warmup_timeouts():
    ...
    for instance in StrategyInstance.objects.filter(enabled=True,state="WARMING_UP").select_related("instrument"):
        ...
        if not subscription:reason="no broker market-data subscription was created"
        elif subscription.state=="ERROR":reason=subscription.last_error or "broker market-data subscription failed"
        else:
            ...
            elif subscription.last_event_at is None:reason=f"no IBKR historical or live event received (subscription {subscription.state})"
