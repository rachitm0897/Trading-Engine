...
from apps.broker_gateway.client import GatewayClient, GatewayRoute, GatewayRouteError
...
from apps.broker_gateway.services import delete_session, provision_session, synchronize_accounts
...
def test_qch_payload_and_bearer_auth_do_not_leak_into_error():
    url = "https://qch.example/api/v1/apps/app-1/subcontainers"
    responses.post(url, status=201, json={"data": {"id": "child-1", "name": "session-child", "status": "RUNNING"}})
    qch = QCHBrokerClient()
    child = qch.create_container(
        name="session-child", image="registry/ibkr@sha256:abc", command=["/app/entrypoint.sh"],
        environment={"IB_USERNAME": "secret-user", "IB_PASSWORD": "secret-password"}, network="traefik",
    )
    request = responses.calls[0].request
    payload = json.loads(request.body)
    assert child.id == "child-1"
    assert request.headers["Authorization"] == "Bearer qch-secret"
    assert payload["network"] == "traefik" and payload["command"] == ["/app/entrypoint.sh"]
    assert payload["environment"]["IB_PASSWORD"] == "secret-password"
    ...
    with pytest.raises(QCHError) as error:
        qch.create_container(
            name="session-child-2", image="registry/ibkr@sha256:abc", command=["/app/entrypoint.sh"],
            environment={"IB_PASSWORD": "secret-password"}, network="traefik",
        )
...
def test_provision_consumes_secret_builds_required_environment_and_live_waits_for_2fa(monkeypatch):
    ...
    environment = qch.created[0]["environment"]
...
def test_provision_retry_adopts_after_qch_409_and_failure_always_deletes_secret(monkeypatch):
    adopted = make_session("Adopt", "paper")
    add_secret(adopted)
    child = QCHContainer("existing-id", adopted.child_container_name, "RUNNING", {})
    qch = FakeQCH(existing=child, conflict=True)
    monkeypatch.setattr("apps.broker_gateway.services.requests.get", lambda *args, **kwargs: HealthyResponse())
    ...
    assert adopted.child_container_id == "existing-id" and len(qch.created) == 1

    failed = make_session("Failed", "paper")
    add_secret(failed)
    broken = FakeQCH()
    broken.create_container = lambda **kwargs: (_ for _ in ()).throw(QCHError("provision failed"))
    assert provision_session(failed.pk, qch_client=broken) == BrokerGatewaySession.Status.ERROR
...
def test_reconnect_and_idempotent_delete_leave_other_session_intact(client, monkeypatch):
    ...
    first.child_container_id = "delete-me"; first.save(update_fields=["child_container_id"])
    ...
    assert qch.deleted == ["delete-me"] and second.status == "CONNECTED"
